...
public class Unit11Lab3 {
	public static double calculateAverage( ArrayList<Pets> list )
	{
		...
		//keeping track of total so i can calculate average later
		int total = 0;
		
		//keeping track of size so i can calculate average later
		int size = list.size();
		
		//looping through and adding to total 
		for (Pets p: list) {
			total += p.age;
		}
	...
	public static void main(String[] args) {
		...
		for (int i = 1; i <= dogs; i++) {
			...
			pets.add(new Dog(name, weight, age));
		...
		for (int i = 1; i <= cats; i++) {
			...
			pets.add(new Cat(name, weight, age));
		...
		//calling average
		System.out.println("");
		System.out.println("Average Age is " + calculateAverage(pets));
